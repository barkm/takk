...
import { framing, FRAMING, LOW, NARROW, WIDE } from "$lib/framing";
 ...
 * with the head at `head` and the shoulders at `shoulders` down the frame. */
function frame({ width = 0.3, centre = 0.5, head = 0.2, shoulders = 0.4 } = {}): Float32Array {
  ...
  put(SHOULDERS[0], centre + width / 2, shoulders);
  put(SHOULDERS[1], centre - width / 2, shoulders);
...
describe("framing", () => {
  ...
  it("says which way to move", () => {
    ...
    expect(framing(frame({ head: 0.4, shoulders: LOW + 0.05 }))).toBe(FRAMING.low); // the upper body cut off
  });
});
