...
export function hear(ears: Ears, level: number, settings = SETTINGS): Ears {
  if (ears.phase === "calibrating") {
    ...
    const middle = [...floor].sort((a, b) => a - b)[
      Math.floor(floor.length / 2)
    ];
    return {
      ...ears,
      floor,
      phase: "armed",
      threshold: Math.max(middle * settings.over, settings.least),
    };
  }
  if (ears.phase === "armed") {
    ...
    return {
      ...ears,
      loud,
      phase: loud >= settings.loud ? "speaking" : "armed",
    };
  }
  ...
  return {
    ...ears,
    quiet,
    phase: quiet >= settings.quiet ? "done" : ears.phase,
  };
}
