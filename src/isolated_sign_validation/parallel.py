...
def parallel_map[T, R](
    ...
    is slower than the workers. `fn` must be a module-level function. Uses one worker per CPU
    unless `max_workers` is given.
    """
    ctx = multiprocessing.get_context("spawn")  # polars and mediapipe are not fork-safe
    with ProcessPoolExecutor(max_workers=max_workers or os.cpu_count(), mp_context=ctx) as pool:
