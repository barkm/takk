...
- **Code layout.** Two packages. `src/isolated_sign_validation/` is the verification problem: common landmark format in `landmarks.py`, MediaPipe extraction in `extraction.py`, one adapter per dataset in `datasets/` (ASL Citizen, MM-WLAuslan, Slovo, WLASL, Svenskt teckenspråkslexikon, ASL-LEX, own recordings, Kaggle ASL Signs), splits in `splits.py`, training data preparation in `preparation.py` (cached in `data/prepared/`), PyTorch dataset and augmentation in `dataset.py`, models in `models.py`, the training loop in `training.py`, k-shot verification evaluation in `evaluation.py` and `baselines.py`, the browser recording app in `collection.py` (its pages in `web/`). `src/takk/` is the TAKK practice app (`uv run takk`, a FastAPI API only), which imports the other package; nothing imports it back (`tests/test_layout.py`). Its page is a separate SvelteKit app in `frontend/`, deployed apart from the API and never served by it (`npm run dev`, `npm run check`, `npm test`). One-off scripts in `scripts/`, tests in `tests/` with the app's in `tests/takk/` (`uv run pytest`).
...
- **Swedish Sign Language is the goal, and already in use:** Svenskt teckenspråkslexikon (teckensprakslexikon.su.se) is an evaluation set and the practice app's glossary, never training data.
