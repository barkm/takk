...
- **Code layout.** Library code in `src/isolated_sign_validation/`: common landmark format in `landmarks.py`, one adapter per dataset in `datasets/`, splits in `splits.py`, training data preparation in `preparation.py` (cached in `data/prepared/`), PyTorch dataset and augmentation in `dataset.py`. One-off scripts in `scripts/`, tests in `tests/` (`uv run pytest`).
